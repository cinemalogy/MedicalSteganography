...
Public Class AdmissionForm
    ...
    Private Sub EmpDetForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd3 As New SqlCommand("select RoomNo from roomTab order by roomNo", Conn)
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd4 As New SqlCommand("select DrCode from DoctorTab order by DrCode", Conn)
    ...
    Sub SaveRecord()
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        q1Var = "insert into AdmissionTab("
        q2Var = " values("
        q1Var = q1Var & "RegNo" & ","
    ...
    Sub disRecords()
        ...
        Dim adp As New SqlDataAdapter("Select RegNo,ipName From AdmissionTab order by RegNo", Conn)
        adp.Fill(DS1)
    ...
    Private Sub DG1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG1.CellContentClick
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select * from AdmissionTab where RegNo=" & pkVar & "", Conn)
    ...
    Private Sub butModify_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles butModify.Click
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim cmd1 As New SqlCommand("Delete from AdmissionTab where RegNo=" & pkVar & "", Conn)
        cmd1.ExecuteNonQuery()
        If Conn.State = ConnectionState.Open Then Conn.Close()
        SaveRecord()
        butNew.Enabled = True
    ...
    Private Sub butNew_Click1(ByVal sender As Object, ByVal e As System.EventArgs) Handles butNew.Click
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select max(RegNo) from AdmissionTab", Conn)
        Dim D1 As SqlDataReader = Cmd0.ExecuteReader()
        If D1.Read Then
