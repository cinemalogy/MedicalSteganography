...
Public Class mGroupForm
    Dim pkVar As String
    Private Sub mGroupForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Sub SaveRecord()
        ...
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select GroupName from GroupTab where GroupName='" & UCase(TextBox1.Text) & "'", Conn)
        ...
        MsgBox("group saved successfully")
        If Conn.State = ConnectionState.Open Then Conn.Close()
        disRecords()
    ...
    Sub disRecords()
        ...
        Dim adp As New SqlDataAdapter("Select GroupName,Details From GroupTab order by GroupName", Conn)
        adp.Fill(DS1)
    ...
    Private Sub DG1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG1.CellContentClick
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select * from GroupTab where GroupName='" & pkVar & "'", Conn)
        Dim D1 As SqlDataReader = Cmd0.ExecuteReader()
        If D1.HasRows Then
            D1.Read()
    ...
    Private Sub butModify_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles butModify.Click
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim cmd1 As New SqlCommand("Delete from GroupTab where GroupName='" & pkVar & "'", Conn)
        cmd1.ExecuteNonQuery()
        If Conn.State = ConnectionState.Open Then Conn.Close()
        SaveRecord()
        butNew.Enabled = True
