...
Public Class DeptForm
    ...
    Sub SaveRecord()
        ...
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select Depttab from DeptTab where Depttab='" & UCase(TextBox1.Text) & "'", Conn)
    ...
    Sub disRecords()
        ...
        Dim adp As New SqlDataAdapter("Select Depttab,Details From DeptTab order by Depttab", Conn)
        adp.Fill(DS1)
    ...
    Private Sub DG1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG1.CellContentClick
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select * from DeptTab where Depttab='" & pkVar & "'", Conn)
        Dim D1 As SqlDataReader = Cmd0.ExecuteReader()
        If D1.HasRows Then
            D1.Read()
    ...
    Private Sub butModify_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles butModify.Click
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim cmd1 As New SqlCommand("Delete from DeptTab where Depttab='" & pkVar & "'", Conn)
        cmd1.ExecuteNonQuery()
        If Conn.State = ConnectionState.Open Then Conn.Close()
        SaveRecord()
        butNew.Enabled = True
