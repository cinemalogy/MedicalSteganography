...
Public Class OutPatientDetailsForm
    Dim pkVar As Long
    ...
    Sub SaveRecord()
        ...
        Conn.Open()
        q1Var = "insert into OutPatientTab("
        q2Var = " values("
        q1Var = q1Var & "opCode" & ","
        q2Var = q2Var & "" & pkVar & ","
        q1Var = q1Var & "opName" & ","
    ...
    Sub disRecords()
        ...
        Dim adp As New SqlDataAdapter("Select opCode,opName From OutPatientTab order by opCode", Conn)
        adp.Fill(DS1)
    ...
    Private Sub DG1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG1.CellContentClick
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select * from OutPatientTab where opCode='" & pkVar & "'", Conn)
    ...
    Private Sub butModify_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles butModify.Click
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim cmd1 As New SqlCommand("Delete from OutPatientTab where opCode='" & pkVar & "'", Conn)
        cmd1.ExecuteNonQuery()
        If Conn.State = ConnectionState.Open Then Conn.Close()
        SaveRecord()
        butNew.Enabled = True
    ...
    Private Sub butNew_Click1(ByVal sender As Object, ByVal e As System.EventArgs) Handles butNew.Click
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select max(opCode) from OutPatientTab", Conn)
        Dim D1 As SqlDataReader = Cmd0.ExecuteReader()
        If D1.Read Then
