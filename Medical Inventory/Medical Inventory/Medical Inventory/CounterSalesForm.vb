...
Public Class CounterSalesForm
    Dim pkVar, BillNoVar As Long
    Dim rowNo As Long
    Dim totAmt, taxVar As Decimal
    Dim DS2 As New DataSet
    Dim Tab1 As DataTable
    Private Sub CounterSalesForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Sub disRecords()
        ...
        Dim adp As New SqlDataAdapter("Select Billno,PartyCode From SaMain where btype = 4 order by Billno", Conn)
        adp.Fill(DS1)
    ...
    Private Sub ComboBox2_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles ComboBox2.Leave
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select sPrice,TaxPer from ProductTab where ProdCode='" & ComboBox2.Text & "'", Conn)
        Dim D1 As SqlDataReader = Cmd0.ExecuteReader()
        If D1.HasRows Then
            D1.Read()
